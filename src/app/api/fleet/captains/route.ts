...
export async function GET(request: NextRequest) {
  try {
    ...
    const captains = await queryMany(
      `SELECT su.id, su.first_name, su.last_name, su.email, su.role, su.is_active,
              ...
              (SELECT COUNT(*) FROM fleet_assignments fa
               WHERE fa.captain_id = su.id AND fa.date = CURRENT_DATE AND fa.status != 'cancelled') as today_assignments,
              (SELECT COUNT(*) FROM fleet_assignments fa
               ...
               AND fa.status = 'completed') as month_completed_trips
       FROM staff_users su
    ...
    if (action === 'add_skill') {
      const skill = await queryOne(
        `INSERT INTO captain_skills (staff_id, skill, certified_at, expires_at, notes)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (staff_id, skill) DO UPDATE SET certified_at = $3, expires_at = $4, notes = $5
         RETURNING *`,
